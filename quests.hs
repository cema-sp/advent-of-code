...
-- newtype BoxParser a = BP (String -> Maybe (a, String))

-- instance Functor BoxParser where
--   fmap = liftM

-- instance Applicative BoxParser where
--   pure x       = BP (\inp -> Just (x, inp))
--   (<*>)  = ap

-- instance Monad BoxParser where
--   (BP f) >>= g = BP (\inp -> case f inp of
--                                   Nothing        -> Nothing
--                                   Just (x, inp') -> bParse (g x) inp')

-- instance Alternative BoxParser where
--   empty = BP (\_ -> Nothing)

-- instance MonadPlus BoxParser

-- bFailure :: BoxParser a
-- bFailure = mzero

-- bParse :: BoxParser a -> String -> Maybe (a, String)
-- bParse (BP p) inp = p inp

-- bItem :: BoxParser Char
-- bItem = BP (\inp -> case inp of
--                         []     -> Nothing
--                         (x:xs) -> Just (x, xs))

-- bDim :: BoxParser Int
-- bDim = do x <- bItem
--           if isDigit x then return ((read x) + bDim)
